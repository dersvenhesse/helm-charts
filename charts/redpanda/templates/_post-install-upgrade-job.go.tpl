...
{{- break -}}
{{- end -}}
{{- $mounts := (get (fromJson (include "redpanda.DefaultMounts" (dict "a" (list $dot) ))) "r") -}}
{{- $volumes := (get (fromJson (include "redpanda.DefaultVolumes" (dict "a" (list $dot) ))) "r") -}}
{{- $vol_1 := (get (fromJson (include "redpanda.Listeners.TrustStoreVolume" (dict "a" (list $values.listeners $values.tls) ))) "r") -}}
{{- if (ne $vol_1 (coalesce nil)) -}}
...
{{- $values := $dot.Values.AsMap -}}
{{- $envars := (list ) -}}
{{- $license_2 := (get (fromJson (include "redpanda.GetLicenseLiteral" (dict "a" (list $dot) ))) "r") -}}
{{- $secretReference_3 := (get (fromJson (include "redpanda.GetLicenseSecretReference" (dict "a" (list $dot) ))) "r") -}}
{{- if (ne $license_2 "") -}}
{{- $envars = (concat (default (list ) $envars) (list (mustMergeOverwrite (dict "name" "" ) (dict "name" "REDPANDA_LICENSE" "value" $license_2 )))) -}}
{{- else -}}{{- if (ne $secretReference_3 (coalesce nil)) -}}
{{- $envars = (concat (default (list ) $envars) (list (mustMergeOverwrite (dict "name" "" ) (dict "name" "REDPANDA_LICENSE" "valueFrom" (mustMergeOverwrite (dict ) (dict "secretKeyRef" $secretReference_3 )) )))) -}}
{{- end -}}
{{- end -}}
...
{{- $ok_5 := $tmp_tuple_3.T2 -}}
{{- $str_4 := $tmp_tuple_3.T1 -}}
{{- if $ok_5 -}}
{{- $envars = (concat (default (list ) $envars) (list (mustMergeOverwrite (dict "name" "" ) (dict "name" (printf "RPK_%s" (upper $k)) "value" $str_4 )))) -}}
{{- else -}}
...
{{- $ok_7 := $tmp_tuple_4.T2 -}}
{{- $v_6 := $tmp_tuple_4.T1 -}}
{{- $ak_8 := $values.storage.tiered.credentialsSecretRef.accessKey -}}
{{- if (and $ok_7 (ne $v_6 "")) -}}
{{- $_is_returning = true -}}
{{- (dict "r" (list (mustMergeOverwrite (dict "name" "" ) (dict "name" "RPK_CLOUD_STORAGE_ACCESS_KEY" "value" (get (fromJson (include "_shims.typeassertion" (dict "a" (list "string" $v_6) ))) "r") )))) | toJson -}}
{{- break -}}
{{- else -}}{{- if (get (fromJson (include "redpanda.SecretRef.IsValid" (dict "a" (list $ak_8) ))) "r") -}}
{{- $_is_returning = true -}}
{{- (dict "r" (list (mustMergeOverwrite (dict "name" "" ) (dict "name" "RPK_CLOUD_STORAGE_ACCESS_KEY" "valueFrom" (mustMergeOverwrite (dict ) (dict "secretKeyRef" (mustMergeOverwrite (dict "key" "" ) (mustMergeOverwrite (dict ) (dict "name" $ak_8.name )) (dict "key" $ak_8.key )) )) )))) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}
{{- $_is_returning = true -}}
...
{{- $v_9 := $tmp_tuple_5.T1 -}}
{{- $sk_11 := $values.storage.tiered.credentialsSecretRef.secretKey -}}
{{- if (and $ok_10 (ne $v_9 "")) -}}
{{- $_is_returning = true -}}
{{- (dict "r" (list (mustMergeOverwrite (dict "name" "" ) (dict "name" "RPK_CLOUD_STORAGE_SECRET_KEY" "value" (get (fromJson (include "_shims.typeassertion" (dict "a" (list "string" $v_9) ))) "r") )))) | toJson -}}
{{- break -}}
{{- else -}}{{- if (get (fromJson (include "redpanda.SecretRef.IsValid" (dict "a" (list $sk_11) ))) "r") -}}
{{- $_is_returning = true -}}
{{- (dict "r" (list (mustMergeOverwrite (dict "name" "" ) (dict "name" "RPK_CLOUD_STORAGE_SECRET_KEY" "valueFrom" (mustMergeOverwrite (dict ) (dict "secretKeyRef" (mustMergeOverwrite (dict "key" "" ) (mustMergeOverwrite (dict ) (dict "name" $sk_11.name )) (dict "key" $sk_11.key )) )) )))) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}
{{- $_is_returning = true -}}
...
{{- $ok_13 := $tmp_tuple_6.T2 -}}
{{- $v_12 := $tmp_tuple_6.T1 -}}
{{- $sk_14 := $values.storage.tiered.credentialsSecretRef.secretKey -}}
{{- if (and $ok_13 (ne $v_12 "")) -}}
{{- $_is_returning = true -}}
{{- (dict "r" (list (mustMergeOverwrite (dict "name" "" ) (dict "name" "RPK_CLOUD_STORAGE_AZURE_SHARED_KEY" "value" (get (fromJson (include "_shims.typeassertion" (dict "a" (list "string" $v_12) ))) "r") )))) | toJson -}}
{{- break -}}
{{- else -}}{{- if (get (fromJson (include "redpanda.SecretRef.IsValid" (dict "a" (list $sk_14) ))) "r") -}}
{{- $_is_returning = true -}}
{{- (dict "r" (list (mustMergeOverwrite (dict "name" "" ) (dict "name" "RPK_CLOUD_STORAGE_AZURE_SHARED_KEY" "valueFrom" (mustMergeOverwrite (dict ) (dict "secretKeyRef" (mustMergeOverwrite (dict "key" "" ) (mustMergeOverwrite (dict ) (dict "name" $sk_14.name )) (dict "key" $sk_14.key )) )) )))) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}
{{- $_is_returning = true -}}
